...
async function getData() {
  ...
  const formatLabels = (labels) => {
    ...
    return labels.map((label) => `<span class="badge badge-label text-uppercase">${label}</span>`).join("");
  };
  ...
  data.menu.forEach((category) => {
    ...
    category.items.forEach((item) => {
      ...
      const formattedExtras = formatExtras(item.additions);
      output += `
        <div class="col-12 col-sm-6 col-md-6 col-lg-4 d-flex flex-column justify-content-between cursor-pointer" 
             data-bs-toggle="modal" 
             data-bs-target="#itemModal" 
             data-bs-image="${item.imageUrl ? `/img/menu/${item.imageUrl}` : "/img/cover-placeholder.jpg"}" 
             data-bs-name="${item.name}" 
             data-bs-description="${item.description}" 
             data-bs-price="${formattedPrice}">
            ${
              item.image
                ? `<img class="menu-item-image img-fluid mb-3" 
                 src="/img/menu/${item.imageUrl}" 
                 alt="${item.name}" 
                 loading="lazy" 
                 onError="this.onerror=null;this.src='/img/placeholder-burger.png';" />`
                : ""
            }
          <div class="flex-grow-1">
            <div class="d-flex align-items-center">
              ${item.name ? `<p class="food-title mb-1 me-2">${item.name}</p>` : ""}
